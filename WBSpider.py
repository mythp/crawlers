...
cookie = 'SUB=_2A25ODwaEDeRhGedJ71cV8ynNwj2IHXVt86rMrDV6PUJbkdAKLUTGkW1NVhyqRH7i9iR35768EV_p9zm-k9VF_X_7; _T_WM=91203037256; MLOGIN=1; M_WEIBOCN_PARAMS=oid%3D4785707983832954%26luicode%3D20000061%26lfid%3D4785707983832954; WEIBOCN_FROM=1110105030'
...
def body(h_1):  # 主体
    ...
    zanshu=[]
    for i in names_0:
        aa = re.search(re.compile('\d+'), i)
        i = re.sub(pattern, "", i)
        if(i=='赞[]'):
            zanshu.append(aa.group())
        if i not in ma:
            if '@' not in i:
                names.append(i)
            if '@' in i:
                backnames.append(i.replace('@',''))
            else:
                backnames.append('')
    ...
    times = []  # 时间
    fromsource = []  # 时间
    pattern_1 = re.compile(r'\d{2}月\d{2}日 \d{2}:\d{2}')  # 匹配日期
    pattern_2 = re.compile(r'来自([\u4e00-\u9fa5]+)')  # 匹配日期
    for i in times_0:
        try:
            t_1 = re.match(pattern_1, i).group()
            t_2 = re.search(pattern_2, i).group()
        except:
            a = datetime.datetime.now().strftime('%m%d')
            t_1 = a[:2] + '月' + a[2:] + '日'  # 改为当天
        times.append(t_1)
        fromsource.append(t_2.replace('来自',''))

    all = []
    for i in range(len(user_ids)):  # 这有问题
        try:
            al = [user_ids[i], names[i],backnames[i],  contents[i], times[i]],zanshu[i],fromsource[i]
        except:
            j = '空'
            contents.append(j)
            al = [user_ids[i], names[i],backnames[i], contents[i], times[i]],zanshu[i],fromsource[i]
...
def save_afile(alls, filename):
    ...
    sheet1.write(0, 2, '被回复用户名')
    sheet1.write(0, 3, '评论内容')
    sheet1.write(0, 4, '时间')
    sheet1.write(0, 5, '点赞数')
    sheet1.write(0, 6, '来源')
    i = 1
    for all in alls:
        for data in all:
            ...
            i = i + 1
    f.save(r'F:\pythonwork\WeiBo/' + filename + '.xls')
